...
captured_frame = None  # The frozen captured image
in_capture_mode = False  # Whether we're viewing a captured image
calibration_mode_active = False  # Whether we're calibrating the capture
...
def draw_manual_calibration_mode(frame):
    ...
    instructions = [
        "1. Click and drag to match the",
        "   reference line (3 inch / 7.62 cm)",
        "2. Click APPLY when aligned"
    ]
    ...
    cv2.putText(overlay, "Reference: 3 inch (76.2 mm)",
                (MANUAL_PANEL_X + 60, MANUAL_PANEL_Y + 145),
    ...
    cv2.putText(overlay, f"Current: {PIXELS_PER_MM:.2f} px/mm",
                (MANUAL_PANEL_X + 20, MANUAL_PANEL_Y + 240),
...
def draw_ui(frame, yolo_objects, active_zone_box, pellets, tick_data, is_captured):
    # Draw Objects (only during calibration)
    ...
    # Draw Ticks (only during calibration)
    if calibration_mode_active and tick_data:
