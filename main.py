...
# ----------------------------------------------------------------------
# Main Overlay (UPDATED LABEL STYLE)
# ----------------------------------------------------------------------
def draw_overlay(frame, pellets):
    ...
    for p in pellets:
        ...
        # --- UPDATED LABEL STYLE START ---
        # Using Moments to calculate center for text (mimicking reference style)
        M = cv2.moments(box)
        if M["m00"] != 0:
